...
public interface EmrVisitRepo extends GenericRepository<EmrVisit> {
	...
	@Query(value = "SELECT DISTINCT visit FROM EmrVisit visit "
			...
			+ "LEFT JOIN FETCH sample.lkpContainerType lct "
			+ "LEFT JOIN FETCH lct.containerImage "
			+ "LEFT JOIN FETCH sample.labTestActualSet lta "
			+ "LEFT JOIN FETCH lta.sourceActualTest "
	...
	@Query("SELECT DISTINCT visit FROM EmrVisit visit "
			+ "LEFT JOIN FETCH visit.emrPatientInfo "
			+ "LEFT JOIN FETCH visit.visitType "
			+ "LEFT JOIN FETCH visit.providerPlan pp "
			+ "LEFT JOIN FETCH pp.insProvider "
			+ "LEFT JOIN FETCH visit.labSamples ls "
			+ "LEFT JOIN FETCH ls.labTestActualSet lta "
			+ "LEFT JOIN FETCH lta.sourceActualTest "
			+ "LEFT JOIN FETCH lta.lkpOperationStatus "
			+ "LEFT JOIN FETCH lta.testDefinition "
			+ "LEFT JOIN FETCH lta.testDestination dest "
			+ "LEFT JOIN FETCH dest.destinationBranch "
			+ "LEFT JOIN FETCH dest.type "
			+ "LEFT JOIN FETCH lta.billChargeSlipList bcs "
			+ "LEFT JOIN FETCH bcs.billPatientTransactionList bpt "
			+ "LEFT JOIN FETCH bpt.lkpTransactionType "
			+ "WHERE "
			+ "visit.rid = :visitRid")
	EmrVisit getCancelData(@Param("visitRid") Long visitRid);
