...
public class EmrVisitGroupService extends GenericService<EmrVisitGroup, EmrVisitGroupRepo> {
	...
	public List<EmrVisitGroup> updateVisitGroups(List<EmrVisitGroup> visitGroups) {
		return getRepository().save(visitGroups);
	}

	public List<EmrVisitGroup> getByVisitsRid(List<Long> visitsRid) {
		return getRepository().find(Arrays.asList(new SearchCriterion("emrVisit.rid", visitsRid, FilterOperator.in)), EmrVisitGroup.class,
				"emrVisit");
	}
