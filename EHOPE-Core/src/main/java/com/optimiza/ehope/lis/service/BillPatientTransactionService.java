...
public class BillPatientTransactionService extends GenericService<BillPatientTransaction, BillPatientTransactionRepo> {
	 ...
	 * 
	 * @param visitRid
	 * @param testActualsRid : list of LabTestActual to cancel.
	 */
	public void cancelPayment(Long visitRid, String cancelReason, List<Long> testActualsRid) {
		...
		EmrVisit visit = visitService.findOne(
				...
				"providerPlan.insProvider");
		LabBranch labBranch = branchService.findById(visit.getBranchId());
		List<BillChargeSlip> visitChargeSlips = billChargeSlipService.getCancelData(visitRid);
		...
		for (BillChargeSlip bcs : visitChargeSlips) {
			...
			if (!testActualsRid.contains(lta.getRid()) || bcs.getIsCancelled()) {
				continue;
			}
	...
	public void payment(TestPricingWrapper testPricingWrapper) {
		...
		List<PaymentWrapper> paymentWrapperPricingList = (List<PaymentWrapper>) map.get("result");
		...
		List<BillChargeSlip> chargeSlips = billChargeSlipService.chargeBillMasterItems(visit, paymentWrapperPricingList,
				testPricingWrapper);
	 ...
	 */
	public void areTestsWithoutBill(Set<TestDefinition> testDefinitionList, List<BillTestItem> billTestItemList) {
	 ...
	 */
	private CalculationData getCalculationData(List<TestDefinition> selectedTests, List<TestGroup> selectedGroups,
		...
		Set<TestDefinition> testDefinitions = new HashSet<>(selectedTests);
		...
		List<BillTestItem> billTestItemList = billTestItemService.getByTestDefinitions(testDefinitions);
		...
		areTestsWithoutBill(testDefinitions, billTestItemList);
		...
		Set<PaymentWrapper> paymentWrapperList = new HashSet<>();

		// payment wrapper equals is using TestDefinition's equals, which means that we will only add one payment wrapper for each test definition
		for (BillTestItem bti : billTestItemList) {
			PaymentWrapper pw = new PaymentWrapper();
			TestDefinition td = ReflectionUtil.unproxy(bti.getTestDefinition());
			bti.setTestDefinition(td);
			pw.setTestDefinition(td);
			paymentWrapperList.add(pw);
		...
		for (BillMasterItem bmi : billMasterItemList) {
			...
			for (PaymentWrapper pw : paymentWrapperList) {
				for (BillTestItem bti : billTestItemList) {
					if (bti.getTestDefinition().getRid().equals(pw.getTestDefinition().getRid())
							&& bti.getBillMasterItem().getRid().equals(bmi.getRid())) {
						PaymentInformation pi = new PaymentInformation();
						pi.setBillMasterItem(bmi);
						// payment information equals is using BillMasterItem's equals, which means that we will only add payment information for each bill master item
						pw.addItemToPaymentInformationList(pi);
					}
				}
			}
		}

		return new CalculationData(fetchedBmiList, bmiNoPriceList, defaultBillPriceList, userInsProviderPlan,
				new ArrayList<>(paymentWrapperList), fetchedTestGroups);
	...
	public Map<String, Object> getTestsPricing(Long visitRid, TestPricingWrapper testPricingWrapper) {
		...
		List<TestDefinition> testDefinitionList = visit	.getLabSamples().stream().flatMap(s -> s.getLabTestActualSet().stream())
														.filter(lta -> !excludedStatuses.contains(lta.getLkpOperationStatus().getCode()))
														.distinct()
														.map(LabTestActual::getTestDefinition)
														.collect(Collectors.toList());

		testPricingWrapper.setTestDefinitionList(testDefinitionList);
		...
		List<PaymentInformation> paymentAuthorities = new ArrayList<>();
		...
		for (PaymentWrapper pw : testPricingWrapper.getPaymentWrapperList()) {
			for (PaymentInformation pi : pw.getPaymentInformationList()) {
				paymentAuthorities.add(pi);
			}
		}

		CalculationData calculationData = getCalculationData(testDefinitionList, selectedGroups, userInsProviderPlan);

		List<PaymentWrapper> paymentWrapperList = calculationData.getPaymentWrapperList();
		calculate(calculationData, paymentWrapperList, patientInsuranceInfo, paymentAuthorities);

		BigDecimal total = generalDiscount(paymentWrapperList, testPricingWrapper);
		total = total.setScale(2, RoundingMode.HALF_UP);
		return calculationResult(visit, paymentWrapperList, calculationData.getBmiNoPriceList(), total);
	}
	 ...
	 */
	private BigDecimal generalDiscount(List<PaymentWrapper> paymentWrapperList, TestPricingWrapper testPricingWrapper) {
		BigDecimal total = BigDecimal.ZERO;
		for (PaymentWrapper pw : paymentWrapperList) {
			total = total.add(pw.getPaymentInfoTotal());
		}
		...
		if (generalPercentage != null) {
			...
			for (PaymentWrapper pw : paymentWrapperList) {
				pw.setPaymentInfoTotal(BigDecimal.ZERO);//reset
				for (PaymentInformation pi : pw.getPaymentInformationList()) {
					BigDecimal charge = pi.getCharge();
					charge = charge.subtract(charge.multiply(generalPercentage));
					charge = amountRounding(charge);
					pi.setCharge(charge);
					pw.setPaymentInfoTotal(pw.getPaymentInfoTotal().add(charge));
				}
			}
	...
	public Map<String, Object> getTestsPricingNoVisit(TestPricingWrapper testPricingWrapper) {
		...
		CalculationData calculationData = getCalculationData(testDefinitionList, testPricingWrapper.getTestGroupList(),
				userInsProviderPlan);

		List<PaymentWrapper> paymentWrapperList = calculationData.getPaymentWrapperList();
		calculate(calculationData, paymentWrapperList, null, new ArrayList<>());
		...
		total = total.setScale(2, RoundingMode.HALF_UP);
		return calculationResult(null, paymentWrapperList, calculationData.getBmiNoPriceList(), total);
	}

	private void calculateFromGroups(PaymentInformation paymentInformation, BillMasterItem bmi, List<TestGroup> testGroups,
		...
		paymentInformation.setInsCoverageDetail(null);
		paymentInformation.setExceededMaxAmount(Boolean.FALSE);
		paymentInformation.setCharge(chosenPrice);
		paymentInformation.setIsAuthorized(Boolean.TRUE);
		paymentInformation.setComment(null);
		paymentInformation.setBillPricing(chosenBillPricing);
		if (chosenGroup != null) {
			...
			if (chosenGroup.getIsProfile() && insuranceBillPricing != null
					&& paymentInformation.getBillPricing().equals(insuranceBillPricing)) {
				paymentInformation.setPercentage(chosenDiscount);
				paymentInformation.setInsCoverageResult(insuranceBillPricing.getPrice().subtract(chosenPrice));
				paymentInformation.setInsDeductionPercentage(userInsProviderPlan.getInsProvider().getDiscount());
				paymentInformation.setInsDeductionResult(
						getInsDeductionResult(userInsProviderPlan.getInsProvider(), paymentInformation.getInsCoverageResult(),
								insuranceBillPricing.getPrice()));
				paymentInformation.setChargeAfterCoverage(chosenPrice);
			} else {

				paymentInformation.setGroupPercentage(chosenGroup.getDiscountPercentage());
				paymentInformation.setGroupAmount(chosenGroup.getDiscountAmount());
				paymentInformation.setGroupTotal(chosenGroup.getTotalPrice());
				paymentInformation.setGroupResult(chosenBillPricing.getPrice().subtract(chosenPrice));
				paymentInformation.setChargeAfterCoverage(chosenPrice);
			}
		}
	}

	private void calculateFromInsurance(PaymentInformation paymentInformation, InsProviderPlan userInsProviderPlan,
			BillPricing insuranceBillPricing, EmrPatientInsuranceInfo patientInsInfo, Map<InsCoverageDetail, BigDecimal> insCovMaxAmtMap) {

		BillMasterItem masterItem = paymentInformation.getBillMasterItem();
		...
		paymentInformation.setInsCoverageDetail(insCoverageDetail);
		paymentInformation.setExceededMaxAmount(exceededMaxAmount);
		paymentInformation.setBillPricing(insuranceBillPricing);
		paymentInformation.setCharge(insPrice);
		paymentInformation.setPercentage(insPercentage);
		paymentInformation.setInsCoverageResult(insuranceBillPricing.getPrice().subtract(insPrice));
		paymentInformation.setInsDeductionPercentage(insDeductionPercentage);
		paymentInformation.setInsDeductionResult(getInsDeductionResult(userInsProviderPlan.getInsProvider(),
				paymentInformation.getInsCoverageResult(), insuranceBillPricing.getPrice()));
		paymentInformation.setChargeAfterCoverage(insPrice);
		paymentInformation.setIsAuthorized(Boolean.TRUE);
	}
	...
	public void calculate(CalculationData calculationData,
			List<PaymentWrapper> paymentWrapperList, EmrPatientInsuranceInfo patientInsInfo,
			List<PaymentInformation> paymentAuthorityList) {
		...
		for (BillMasterItem bmi : fetchedBmiList) {
			...
			PaymentInformation paymentAuthority = paymentAuthorityList	.stream()
																		.filter(pi -> pi.getBillMasterItem().getRid().equals(bmi.getRid()))
																		.findFirst()
																		.orElse(null);
			...
			for (PaymentWrapper pw : paymentWrapperList) {
				...
				List<TestGroup> relatedGroups = testGroups.stream().filter(tg ->
					{
						for (TestGroupDefinition tgd : tg.getGroupDefinitions()) {
							if (tgd.getTestDefinition().getRid().equals(pw.getTestDefinition().getRid())) {
								return true;
							}
						}
						return false;
				...
				for (PaymentInformation pi : pw.getPaymentInformationList()) {
					if (pi.getBillMasterItem().getRid().equals(bmi.getRid())) {
						...
						if (paymentAuthority != null && paymentAuthority.getIsAuthorized() == Boolean.FALSE) {
							calculateUnauthorized(pi, paymentAuthority, defaultBillPricing);
						} else if (!CollectionUtil.isCollectionEmpty(relatedGroups)) {
							calculateFromGroups(pi, bmi, relatedGroups, userInsProviderPlan, defaultBillPricing, insuranceBillPricing,
									patientInsInfo);
						} else if (hasInsurancePricing) {
							calculateFromInsurance(pi, userInsProviderPlan, insuranceBillPricing, patientInsInfo, insCovMaxAmtMap);
						} else {
							pi.setExceededMaxAmount(Boolean.FALSE);
							pi.setBillPricing(defaultBillPricing);
							pi.setCharge(defaultBillPricing.getPrice());
							pi.setChargeAfterCoverage(defaultBillPricing.getPrice());
							pi.setIsAuthorized(Boolean.TRUE);
						}
						pi.setCharge(amountRounding(pi.getCharge()));
						pi.setChargeAfterCoverage(amountRounding(pi.getChargeAfterCoverage()));
						if (pi.getInsCoverageResult() != null) {
							pi.setInsCoverageResult(amountRounding(pi.getInsCoverageResult()));
						}
						if (pi.getInsDeductionResult() != null) {
							pi.setInsDeductionResult(amountRounding(pi.getInsDeductionResult()));
						}
						if (pi.getGroupResult() != null) {
							pi.setGroupResult(amountRounding(pi.getGroupResult()));
						}
						//total amount without general discount deduction
						pw.setPaymentInfoTotal(pw.getPaymentInfoTotal().add(pi.getCharge()));
					}
				}
	...
	private void calculateUnauthorized(PaymentInformation pi, PaymentInformation paymentAuthority, BillPricing defaultPricing) {
		pi.setExceededMaxAmount(Boolean.FALSE);
		pi.setBillPricing(defaultPricing);
		pi.setCharge(defaultPricing.getPrice());
		pi.setChargeAfterCoverage(defaultPricing.getPrice());
		pi.setIsAuthorized(Boolean.FALSE);
		pi.setComment(paymentAuthority.getComment());
	}
	...
	private Stack<InsCoverageDetail> getCovDetailStackByMasterItem(BillMasterItem billMasterItem, InsProviderPlan userInsProviderPlan) {
		...
		for (InsCoverageDetail icd : userInsProviderPlan.getInsCoverageDetailList()) {
			...
			if (icd.getBillClassification() != null && billMasterItem.getBillClassification() != null
				...
				// (3rd level)
				System.out.println(billMasterItem.getRid() + " , %=" + icd.getPercentage() + " , ICD=" + icd.getRid());
	 ...
	 */
	public List<PaymentWrapper> getTestsCoverageDetail(Long visitRid) {
		List<String> excludedStatuses = Arrays.asList(OperationStatus.ABORTED.getValue(), OperationStatus.CANCELLED.getValue());
		EmrVisit visit = visitService.findOne(
				SearchCriterion.generateRidFilter(visitRid, FilterOperator.eq), EmrVisit.class,
				"providerPlan", "visitGroups.testGroup", "labSamples.labTestActualSet.testDefinition",
		...
		CalculationData calculationData = getCalculationData(testDefinitionList, selectedGroups, visit.getProviderPlan());
		List<PaymentWrapper> paymentWrapperList = calculationData.getPaymentWrapperList();
		...
		for (BillMasterItem billMasterItem : calculationData.getFetchedBmiList()) {
			...
			for (PaymentWrapper pw : paymentWrapperList) {
				for (PaymentInformation pi : pw.getPaymentInformationList()) {
					if (pi.getBillMasterItem().getRid().equals(billMasterItem.getRid())) {
						pi.setIsFetched(Boolean.TRUE);
						...
						if (calculationData.getUserInsProviderPlan() != null) {
							...
							if (!CollectionUtil.isCollectionEmpty(insCovByPlanStack)) {
								...
								pi.setInsCoverageDetail(icd);
							}
						}
						if (previousAuth != null) {
							pi.setIsAuthorized(previousAuth);
						} else {
							pi.setIsAuthorized(Boolean.TRUE);
							if (icd != null) {
								pi.setIsAuthorized(!icd.getNeedAuthorization());// if it need authorization then its not authorized, otherwise it is
							}
						}
						break;
					}
	 ...
	 * @return
	 */
	private Map<String, Object> calculationResult(EmrVisit visit, List<PaymentWrapper> paymentWrapperList,
			List<BillMasterItem> bmiNoPriceList,
			BigDecimal total) {
		Map<String, Object> result = new HashMap<>();
		result.put("visit", visit);
		result.put("result", paymentWrapperList);
