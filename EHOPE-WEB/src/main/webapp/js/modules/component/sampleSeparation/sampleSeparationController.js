            ...
            function colorContainers() {
                ...
                for (var index in $scope.samples) {
                    ...
                    if (sample.state === $scope.validSample && sample.labTestActualSet.length > 0 && sample.lkpContainerType != null) {
                        ...
                        var sampleImageId = "#sample_image_" + sample.rid;
                        var imageCode = sample.lkpContainerType.containerImage.code.replace(/_/g, "-").toLowerCase();
                        imageCode = " ." + imageCode + "-cap { fill: " + color + " !important; }";
                        colorRules += sampleImageId + imageCode;
                    } else {
