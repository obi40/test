    ...
    app.directive('testResultEntry', function () {
        return {
            ...
            scope: {
                ...
                noEdit: "=?noEdit",
                filter: "=?filter",
                dialogAction: "=?dialogAction" //use this when in dialog { dialog: <the dialog object>, submit: will be bound below to the save function, reorderTests: will be bound to the reorderTests function}
            },
            ...
            controller: ['$scope', '$timeout', '$q', 'testResultEntryService', 'patientProfileService', 'orderManagementService', 'lovService', 'antiMicrobialService', 'organismService',
                function ($scope, $timeout, $q, testResultEntryService, patientProfileService, orderManagementService, lovService, antiMicrobialService, organismService) {
                    ...
                    $scope.disabledReorderTestsButton = { value: true };
                                                ...
                                                switch (currentActualResult.labResult.resultValueType.code) {
                                                    ...
                                                    case "QN":
                                                        ...
                                                        if (currentActualResult.labResult.isComprehensive) {
                                                            var compCode = testActual.rid + "_" + currentActualResult.labResult.standardCode;
                                                            ...
                                                            var compCode = testActual.rid + "_" + currentActualResult.labResult.comprehensiveResult.standardCode;
                                                            var tempComprehensiveResult = $scope.comprehensiveResults[compCode];
                                                            if (!tempComprehensiveResult) {
                    ...
                    var testIdsToReorder = {};

                    $scope.toReorderChange = function (actualTestRid) {
                        if (testIdsToReorder.hasOwnProperty(actualTestRid)) {
                            delete testIdsToReorder[actualTestRid];
                        } else {
                            testIdsToReorder[actualTestRid] = actualTestRid;
                        }
                        if (Object.keys(testIdsToReorder).length > 0) {
                            $scope.disabledReorderTestsButton.value = false;
                        } else {
                            $scope.disabledReorderTestsButton.value = true;
                        }
                    }
                    ...
                    $scope.reorderTests = function () {
                        testResultEntryService.reorderActualTests(Object.keys(testIdsToReorder))
                            .then(function () {
                    ...
                    function findFormByResultStandardCode(controls, standardCode) {
                        for (var i = 0; i < controls.length; i++) {
                            ...
                            if (control.$name === "differentialValidator" || control.$name === "reorderTest") {
                                continue;
                            }
                    ...
                    $scope.enteringPrimaryResult = function (test, result, primaryDecimals, secondaryDecimals, testForm) {
                        var testDef = test.testDefinition;
                        try {
                            ...
                            if (testDef.interpretations.length > 0) {
                                determineClassFromConcentration(result, testDef.interpretations, primaryDecimals);
                            }
                        } catch (e) {
                        ...
                        if (result.labResult.isComprehensive) {
                            $scope.differentialPercentageChange(test.rid, result.labResult.standardCode, testForm);
                        }
                    };
                    ...
                    if ($scope.dialogAction) {
                        ...
                        $scope.dialogAction.reorderTests = $scope.reorderTests;
                        $scope.dialogAction.disabledReorderTestsButton = $scope.disabledReorderTestsButton;
                    }
                }]
        }
    });
