                    ...
                    function activateDirective() {
                                ...
                                controller: ["$scope", "$mdDialog", "calcualtedPriceDetails", "discount",
                                    function ($scope, $mdDialog, calcualtedPriceDetails, discount) {
                                        ...
                                        for (var idx = 0; idx < $scope.calcualtedPriceDetails.result.length; idx++) {
                                            ...
                                            for (var i = 0; i < pw.chargeSlips.length; i++) {
                                                var pi = pw.chargeSlips[i];
                                                var obj = {};
                                                if (i === 0) {
                                                    obj["piLength"] = pw.chargeSlips.length;
                                                ...
                                                obj["amount"] = pi.amount ? pi.amount : '';
                                                if (pi.percentage) {
                                                    obj["percentage"] = (100 - pi.percentage) + "%";
                                                } else if (pi.billPricing && pi.billPricing.price) {
                                                    obj["percentage"] = commonData.na;
                                                }
                                                if (pi.groupDiscountPercentage) {
                                                    obj["groupDiscountPercentage"] = (100 - pi.groupDiscountPercentage) + "%";
                                                } else if (pi.groupCoverageAmount != null) {
                                                    var p = util.round((pi.groupCoverageAmount / pi.groupTotal) * 100);
                                                    obj["groupDiscountPercentage"] = (100 - p) + "%";
                                                } else if (pi.billPricing && pi.billPricing.price) {
                                                    obj["groupDiscountPercentage"] = commonData.na;
                                                }
                                        ...
                                        function updateUIValues() {
                                            ...
                                            for (var idx = 0; idx < $scope.calcualtedPriceDetails.result.length; idx++) {
                                                if ($scope.calcualtedPriceDetails.result[idx].isPrint) {
                                                    total += $scope.calcualtedPriceDetails.result[idx].chargeSlipsTotal;
                                                    $scope.nonPicked = false;
                                                }
                                            }
                                            $scope.total = total;
                                        ...
                                        $scope.printPayment = function () {
                                            ...
                                            function pricingResultHtml() {
                                                ...
                                                for (var paymentWrapperKey in $scope.calcualtedPriceDetails.result) {
                                                    ...
                                                    for (var payKey in wrapperObj.chargeSlips) {
                                                        var payObj = wrapperObj.chargeSlips[payKey];
                                                        var innerHtml = "<tr>";
                                                        if (payKey == 0) {
                                                            innerHtml = "<tr><td rowspan='" + wrapperObj.chargeSlips.length + "'>" + wrapperObj.testDefinition.description + "</td>";
                                                        }
                                                        ...
                                                        if (payObj.groupDiscountPercentage) {
                                                            innerHtml = innerHtml.concat("<td>" + (100 - util.round(payObj.groupDiscountPercentage)) + "%</td>");
                                                        } else if (payObj.billPricing && payObj.billPricing.price) {
                                                            innerHtml = innerHtml.concat("<td>" + commonData.na + "</td>");
                                                        }

                                                        innerHtml = innerHtml.concat("<td>" + (payObj.amount ? payObj.amount : '') + "</td></tr>");
                        ...
                        $scope.calculatePrice = function () {
                            ...
                            var testPricingWrapper = {
                                testDefinitionList: getToCreateTests(),
                        ...
                        function singleTestSelectability(testDefinition) {
                            ...
                            if ($scope.editOrderTests != null && $scope.editOrderTests.length > 0 && canSelect) {
                                //did we exceed max amount
                                if (testDefinition.isAllowRepetitionDifferentSample) {
                                    if (didTestExceedMaxAmount(testDefinition)) {
                                        canSelect = false;
                                    ...
                                    for (var idx = 0; idx < $scope.editOrderTests.length; idx++) {
                                        var parentTest = $scope.editOrderTests[idx];
                                        if (parentTest.testDefRid === testDefinition.rid) {
                                            for (var i = 0; i < parentTest.children.length; i++) {
                                                var test = parentTest.children[i];
                                                if (!test.isCancelled) {
                                                    canSelect = false;
                                                    cause.push($filter("translate")("selected"));
                                                    break;
                        ...
                        $scope.testRepetitionMaxAmount = function (test) {
                            var maxAmount = test.differentSampleMaxAmount + 1;
                            if ($scope.editOrderTests != null && $scope.editOrderTests.length > 0) {
                                var eot = null;
                                for (var idx = 0; idx < $scope.editOrderTests.length; idx++) {
                                    if (test.rid !== $scope.editOrderTests[idx].testDefRid) {
                                        continue;
                                    }
                                    eot = $scope.editOrderTests[idx];
                                    break;
                                }
                                if (eot != null) {
                                    maxAmount = maxAmount - eot.amount;
                                }
                            }
                            return maxAmount;
                        };
                        function didTestExceedMaxAmount(test) {
                            if ($scope.editOrderTests == null || $scope.editOrderTests.length === 0
                                || !test.isAllowRepetitionDifferentSample) {
                                return false;
                            }
                            var eot = null;
                            for (var idx = 0; idx < $scope.editOrderTests.length; idx++) {
                            ...
                            return (eot.differentSampleMaxAmount + 1) == eot.amount;
                        }
                        ...
                        $scope.toggleTestSelection = function (test, checkFlag) {
                            ...
                            for (var i = 0; i < toggledTests.length; i++) {
                                var toToggleTest = toggledTests[i];
                                var idx = $scope.selectedTestChipsOptions.data.map(function (t) {
                                    return t.rid;
                                }).indexOf(toToggleTest.rid);
                                if (idx > -1) {
                                    //deselect
                                    if (!checkFlag) {
                                        //if checkFlag is true, do not deselect
                                        removeFromTestActualList(toToggleTest);
                                        $scope.selectedTestChipsOptions.data.splice(idx, 1);
                                        toToggleTest.isChecked = false;
                                    }
                                } else {
                                    //select
                                    toToggleTest.isChecked = true;
                                    if (toToggleTest.isAllowRepetitionDifferentSample) {
                                        //keeping old value
                                        toToggleTest.counter = 1;
                                    }
                                    $scope.selectedTestChipsOptions.data.push(toToggleTest);
                                }
                            }
                            if (!checkFlag) {
                                checkSelectedGroups();
                        ...
                        function removeFromTestActualList(test) {
                            //remove all test actuals by this test def rid
                            if ($scope.testActualList == null || $scope.testActualList.length == 0) {
                                return;
                            }
                            var testActualsRid = [];
                            for (var idx = 0; idx < $scope.testActualList.length; idx++) {
                                if (test.rid === $scope.testActualList[idx].testDefinition.rid) {
                                    testActualsRid.push($scope.testActualList[idx].rid);
                                }
                            }
                            if (testActualsRid.length < 1) {
                                return;
                            }
                            testSelectionService.deleteActualTests(testActualsRid).then(function () {
                                var length = $scope.testActualList.length - 1;
                                for (var idx = length; idx >= 0; idx--) {
                                    if (testActualsRid.includes($scope.testActualList[idx].rid)) {
                                        $scope.testActualList.splice(idx, 1);
                                    }
                                }
                            });
                        ...
                        $scope.onTestCounterChange = function (test, oldTestCounter) {
                            //If we decreased the counter delete tests by the difference between them.
                            //this function wont run if counter is 1 and user clicked "decrease" bcz of ng-min
                            //to delete the last test with counter = 1 , click X button
                            if (test.counter == null) {
                                return;
                            }
                            //if not in wizard then run calculatePrice , otherwise run only if decreasing
                            if (!$scope.wiz) {
                                $scope.calculatePrice();
                                return;
                            } else if (test.counter > oldTestCounter || $scope.testActualList == null || $scope.testActualList.length == 0) {
                                return;
                            }
                            var difference = oldTestCounter - test.counter;
                            var testActualsRid = [];
                            for (var idx = 0; idx < $scope.testActualList.length; idx++) {
                                //if we reached the target amount to delete , stop
                                if (testActualsRid.length == difference) {
                                    break;
                                }
                                if (test.rid === $scope.testActualList[idx].testDefinition.rid) {
                                    testActualsRid.push($scope.testActualList[idx].rid);
                        ...
                        function getToCreateTests() {
                            var tests = angular.copy($scope.selectedTestChipsOptions.data);
                            var result = [];
                            //to know how many test def will be created, we will decrease the current counter by it
                            var testsAmount = {};
                            for (var idx = 0; idx < $scope.testActualList.length; idx++) {
                                var testRid = $scope.testActualList[idx].testDefinition.rid;
                                if (testsAmount[testRid] != null) {
                                    testsAmount[testRid] = testsAmount[testRid] + 1;
                                } else {
                                    testsAmount[testRid] = 1;
                                }

                            }
                            for (var idx = 0; idx < tests.length; idx++) {
                                var originalTest = tests[idx];
                                if (!originalTest.counter) {
                                    if (testsAmount[originalTest.rid] == null) {
                                        result.push(originalTest);
                                    }
                                } else {
                                    var counter = originalTest.counter;
                                    if (testsAmount[originalTest.rid] != null) {
                                        counter = counter - testsAmount[originalTest.rid];
                                    }
                                    if (counter > 0) {
                            ...
                            result.sort(function (a, b) {
                                if (a.standardCode < b.standardCode) return -1;
                                if (a.standardCode > b.standardCode) return 1;
                                return 0;
                            });
                            return result;
                        ...
                        $scope.submitSelectedTests = function () {
                            ...
                            var wrapper = {
                                visitRid: $scope.order.rid,
                                testDefinitionList: getToCreateTests(),
                                testGroupList: getSelectedGroups()
                            };
                            //var lastTestActualList = angular.copy($scope.testActualList);
                            if (wrapper.testDefinitionList == null || wrapper.testDefinitionList.length === 0) {
                                exitStep();
                        ...
                        function checkTestsInsPricing() {
                            ...
                            var testsRid = [];
                            if ($scope.selectedTestChipsOptions.data != null && $scope.selectedTestChipsOptions.data.length > 0) {
                                for (var idx = 0; idx < $scope.selectedTestChipsOptions.data.length; idx++) {
                                    testsRid.push($scope.selectedTestChipsOptions.data[idx].rid);
                                }
                            }
                            if ($scope.editOrderTests != null && $scope.editOrderTests.length > 0) {
                                for (var idx = 0; idx < $scope.editOrderTests.length; idx++) {
                                    testsRid.push($scope.editOrderTests[idx].testDefRid);
                                }
                            }
                            if (testsRid.length === 0 || selectedPlan == null) {
                                return;
                            }
