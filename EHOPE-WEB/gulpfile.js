...
var deleteOldDist = function () {
    var oldDist = null;
    fileSys.readdirSync(mainDirectory).filter(fn => fn.startsWith('dist-')).forEach(function (file, index) {
        if (!file) {
            return;
        }
        oldDist = file;
    ...
    deleteFolderRecursive(mainDirectory + oldDist);
};
var deleteFolderRecursive = function (path) {
    if (fileSys.existsSync(path)) {
        fileSys.readdirSync(path).forEach(function (file, index) {
            var curPath = path + "/" + file;
            if (fileSys.lstatSync(curPath).isDirectory()) { // recurse
